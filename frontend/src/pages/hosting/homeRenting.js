...
export default function HomeRenting(){
    ...
    for(let i = 0; i< homeRent.length; i++){
        homeRent[i].orders.map(order => result.push({title: homeRent[i].title, address: homeRent[i].address, email: order.idUser.email, phone: order.idUser.phone, checkin: order.checkin, checkout: order.checkin, status: order.status}));
    }
